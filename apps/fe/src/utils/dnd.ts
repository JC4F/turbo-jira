...
export const insertItemIntoArray = (arr: Issue[], item: Issue, index: number) => {
  const arrClone = [...arr]
  arrClone.splice(index, 0, item)
  return arrClone
}
export const updateArrayItemById = (arr: Issue[], itemId: string, fields: Partial<Issue>) => {
  const arrClone = [...arr]
...
export const getSortedListIssues = (issues: Issue[] | readonly Issue[], status: string) =>
  [...(issues ?? [])]
...
const getAfterDropPrevNextIssue = (
  ...
  const afterDropDestinationIssues = isSameList
    ...
    : insertItemIntoArray(beforeDropDestinationIssues, droppedIssue as Issue, destination.index)
