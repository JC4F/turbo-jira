...
import { createEntity } from '@/common/utils';
import {
  ProjectCategory,
  IssueType,
  IssueStatus,
...
const seedComments = (issues: Issue[], users: User[]): Promise<Comment[]> => {
  const comments = [
    ...
    createEntity(Comment, {
      body: 'The good thing about relationships is that you finally forget how they started.',
      issueId: issues[0].id,
      userId: users[2].id,
    }),
    createEntity(Comment, {
    ...
    createEntity(Comment, {
      body: 'Una mattina mi sono alzato O bella ciao, bella ciao, bella ciao, ciao, ciao',
      issueId: issues[4].id,
      userId: users[1].id,
    }),
    createEntity(Comment, {
      body: 'O partigiano, portami via O bella ciao, bella ciao, bella ciao, ciao, ciao',
      issueId: issues[4].id,
      userId: users[0].id,
    }),
    createEntity(Comment, {
      body: 'There are people who study years to earn a salary, we only go to study for five months',
      issueId: issues[5].id,
      userId: users[1].id,
    }),
    createEntity(Comment, {
      body: 'As in chess, there are times when it is necessary to sacrifice a piece to win',
      issueId: issues[6].id,
      userId: users[2].id,
    }),
  ];
...
export const createGuestAccount = async (): Promise<User> => {
